...
const NavLink = ({setOpen}) => {
  ...
  return (
    <>
      {links.map((link) => (
        <div key={link.name} className="md:border-0 border-b border-[#9e21cc]">
            ...
            {link.submenu && (
              ...
              >
                <div className="absolute top-16 mr-1 hidden group-hover:md:block hover:md:block">
                  <div className="py-3">
                    <div className="w-4 h-4 left-3 absolute mt-1 bg-[#b81eeb] border-t border-l border-[#9021cc] rotate-45"></div>
                  </div>
                  <div className="bg-gradient-to-b from-[#310018] to-[#18f5f5] border border-[#9021cc] rounded-lg p-6 grid grid-cols-3 gap-10">
                    {link.sublinks.map((mysublinks) => (
                      <div key={mysublinks.Head}>
                        <h1 className="text-base pb-2 border-b-2 border-[#b81eeb]">
                          {mysublinks.Head}
                        </h1>
                        <ul className="list-indide list-[square] pl-5">
                          {mysublinks.sublink.map((slink) => (
                            <li
                              key={slink.name}
                              className="text-sm text-white my-3 hover:text-[#b81eeb]"
                            >
                ...
                <div key={slinks.Head}>
                  <div>
                    <h1
                      ...
                      className="py-4 pl-7 font-semibold md:pr-0 pr-5 hover:text-[#8519e3]"
                    >
                      {slinks.Head}
